...
class BaseTrainer(object):
    ...
    def __init__(self):
        ...
        print("trainer args.....", self.params.model)
    ...
    def set_args(self):
        ...
        parser.add_argument(
            '--model', default="transliterationmodel", type=str, help='dump model filename')
        parser.add_argument(
        ...
        parser.add_argument('--cleanup_anyway', default=True,
    ...
    def setup_training(self):
        ...
        if params.optimizer == Optimizer.sgd:
            ...
            self.optimizer = torch.optim.Adadelta(
                self.model.parameters(), params.lr)
    ...
    def reload_and_test(self, model_fp, best_fp, batch_size, decode_fn):
        ...
        if results:
            for result in results:
                self.logger.info(
                    f"DEV {result.long_desc} is {result.res} at epoch -1")
        ...
        if self.data.test_file is not None:
            ...
            results = self.decode(
                TEST, batch_size, f"{model_fp}.decode", decode_fn)
            if results:
                for result in results:
                    self.logger.info(
    ...
    def run(self, start_epoch, decode_fn=None):
        ...
        if finish or params.cleanup_anyway:
            ...
            with torch.no_grad():
                self.reload_and_test(params.model, best_fp,
                                     params.bs, decode_fn)
